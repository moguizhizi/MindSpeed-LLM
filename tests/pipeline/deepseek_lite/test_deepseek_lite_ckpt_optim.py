...
class TestCovertDeepseek2liteckptOptim():
    world_size = 8
    cur_dir = Path(__file__).parent
    json_file = next(cur_dir.glob("*.json"), None)
    test_config = create_testconfig(json_file)

    @pytest.mark.parametrize("params", test_config["test_deepseek_lite_ckpt_optim"])
    def test_deepseek_lite_ckpt_optim(self, build_args, params):
        ...
        arguments.append("--moe-grouped-gemm")
        exit_code = run_cmd(["python3", CKPT_PYPATH] + arguments)
        assert exit_code == 0
        base_dir = "/data/optim_ckpt/deepseek2_lite_pp4_ep2_optim_base"
        save_dir = "/data/optim_ckpt/deepseek2_lite_pp4_ep2_optim_test"
