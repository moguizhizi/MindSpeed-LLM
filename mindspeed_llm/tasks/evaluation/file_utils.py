...
def check_owner(path: str):
    ...
    if not (cur_uid == 0 or cur_uid == path_owner or path_gid == cur_gid):
        raise PermissionError(f"Access denied to the path: {path}. "
                              "You are not the root user, the owner of the path, "
                              "or a member of the same group as the path owner. "
                              "Please ensure you meet at least one of these conditions.")
